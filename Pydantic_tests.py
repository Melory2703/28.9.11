...
def test_users_get_response():
    response = [
        {"id": 234, "first_name": "Oleg", "last_name": "Stepanov"},
        {"id": 457, "first_name": "Anna", "last_name": "Ivanova"}
    ]
...
def test_users_get_success():
    ...
    users = [User(**user) for user in response]
    assert len(users) == 2
    assert users[0].id == 234
    assert users[0].first_name == "Oleg"
    assert users[0].last_name == "Stepanov"
...
def test_users_get_one_user():
    response = [{"id": 234, "first_name": "Oleg", "last_name": "Stepanov666"}]
    users = [User(**user) for user in response]
    assert len(users) == 1
    assert users[0].id == 123
    assert users[0].first_name == "Oleg"
    assert users[0].last_name == "Stepanov666"
